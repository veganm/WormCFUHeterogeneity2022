...
pAU37.b<-subset(All8.big, Host=="AU37") %>%
...
pN2.b<-subset(All8.big, Host=="N2") %>%
	...
	labs(title="N2", y="log10(CFU/Worm)")
pN2.b
pdaf16.b<-subset(All8.big, Host=="daf16") %>%
	ggplot(aes(x=Date, y=logTotal, color=Date)) + geom_jitter(shape=16, position=position_jitter(0.05)) + geom_violin(fill=NA) + 	ylim(1,6) +	theme_classic() + 
	theme(text=element_text(size=14), axis.title.x = element_blank(), legend.position="none", plot.title=element_text(hjust=0.5, size=16)) +
	labs(title=expression(italic("daf-16")), y="log10(CFU/Worm)")
pdaf2.b<-subset(All8.big, Host=="daf2") %>%
	ggplot(aes(x=Date, y=logTotal, color=Date)) + geom_jitter(shape=16, position=position_jitter(0.05)) + geom_violin(fill=NA) + 	ylim(1,6) +	theme_classic() + 
	theme(text=element_text(size=14), axis.title.x = element_blank(), legend.position="none", plot.title=element_text(hjust=0.5, size=16)) +
	labs(title=expression(italic("daf-2")), y="log10(CFU/Worm)")
pdbl1.b<-subset(All8.big, Host=="dbl1") %>%
	ggplot(aes(x=Date, y=logTotal, color=Date)) + geom_jitter(shape=16, position=position_jitter(0.05)) + geom_violin(fill=NA) + 	ylim(1,6) +	theme_classic() + 
	theme(text=element_text(size=14), axis.title.x = element_blank(), legend.position="none", plot.title=element_text(hjust=0.5, size=16)) +
	labs(title=expression(italic("dbl-1")), y="log10(CFU/Worm)")
pdec1.b<-subset(All8.big, Host=="dec1") %>%
	ggplot(aes(x=Date, y=logTotal, color=Date)) + geom_jitter(shape=16, position=position_jitter(0.05)) +
	geom_violin(fill=NA) + 	ylim(1,6) +	theme_classic() + 
	theme(text=element_text(size=14), axis.title.x = element_blank(), legend.position="none", plot.title=element_text(hjust=0.5, size=16)) +
	labs(title=expression(italic("dec-1")), y="log10(CFU/Worm)")
pexp1.b<-subset(All8.big, Host=="exp1") %>%
	ggplot(aes(x=Date, y=logTotal, color=Date)) + geom_jitter(shape=16, position=position_jitter(0.05)) +
	geom_violin(fill=NA) + 	ylim(1,6) +	theme_classic() + 
	theme(text=element_text(size=14), axis.title.x = element_blank(), legend.position="none", plot.title=element_text(hjust=0.5, size=16)) +
	labs(title=expression(italic("exp-1")), y="log10(CFU/Worm)")
pvhp1.b<-subset(All8.big, Host=="vhp1") %>%
	ggplot(aes(x=Date, y=logTotal, color=Date)) + geom_jitter(shape=16, position=position_jitter(0.05)) + geom_violin(fill=NA) + 	ylim(1,6) +	theme_classic() + 
	theme(text=element_text(size=14), axis.title.x = element_blank(), legend.position="none", plot.title=element_text(hjust=0.5, size=16)) +
	labs(title=expression(italic("vhp-1")), y="log10(CFU/Worm)")
pAll8.big<-All8.big %>%
	ggplot(aes(x=as.factor(Host), y=logTotal, color=as.factor(Host)))+
	geom_jitter(shape=16, position=position_jitter(0.05)) +
	geom_violin(fill=NA) + 	ylim(1,6) +	theme_classic() + 
	theme(text=element_text(size=14), axis.title.x = element_blank(), legend.position="none", plot.title=element_text(hjust=0.5, size=16)) +
	labs(title="Minimal Microbiome Totals", y="log10(CFU/Worm)")
plot_grid(pN2.b, pAU37.b, pdaf16.b, pdaf2.b, pdbl1.b, pdec1.b, pexp1.b, pvhp1.b, pAll8.big, labels="AUTO", ncol=3, nrow=3)
...
All8.big.r2r <- data.frame(Host=character(),
                 Date=character(),
                 maxCFU=double(),
                 minCFU=double(),
                 q90=double(),
...
# loop over each host genotype in the data set
# then each replicate (day) for that host
# and record
for (i in 1:length(hosts.big)){
	temp<-unique(All8.big$Date[All8.big$Host==hosts.big[i]])
#	print(hosts.big[i])
	for (j in 1:length(temp)){
#		print(temp[j])
		tempCFU<-All8.big$Total[All8.big$Host==hosts.big[i] & All8.big$Date==temp[j]]
		myq<-as.double(quantile(tempCFU, probs=c(0.1, 0.25, 0.5, 0.75, 0.9)))
		All8.big.r2r<-rbind(All8.big.r2r, data.frame(Host=hosts.big[i],
		Date=temp[j],
		maxCFU=max(tempCFU),
		minCFU=min(tempCFU),
		q90=myq[5],	
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# And we'll add in the Salmonella and S. aureus data
SeCount<-SeCount[complete.cases(SeCount),]
days<-unique(SeCount$Date)
for (i in 1:length(days)){
		tempCFU<-SeCount$Count[SeCount$Date==days[i]]
		myq<-as.double(quantile(tempCFU, probs=c(0.1, 0.25, 0.5, 0.75, 0.9), na.rm=TRUE))
		All8.big.r2r<-rbind(All8.big.r2r, data.frame(Host="SE",
		Date=days[i],
		maxCFU=max(tempCFU),
		minCFU=min(tempCFU),
		q90=myq[5],	
...
days<-unique(SaBleachCount2$Date)
for (i in 1:length(days)){
		tempCFU<-SaBleachCount2$Count[SaBleachCount2$Date==days[i]]
		myq<-as.double(quantile(tempCFU, probs=c(0.1, 0.25, 0.5, 0.75, 0.9), na.rm=TRUE))
		All8.big.r2r<-rbind(All8.big.r2r, data.frame(Host="SA",
		Date=days[i],
		maxCFU=max(tempCFU),
		minCFU=min(tempCFU),
		q90=myq[5],	
...
All8.big.r2r$cvCFU<-sqrt(All8.big.r2r$varCFU)/All8.big.r2r$meanCFU
...
hosts<-unique(NRRL1counts$Condition) #length 24
dim(All8.big.r2r) #26 x 14 before NRRL
names(All8.big.r2r) # make sure the names are in the right order

#make sure there are sufficient replicates
for (i in 1:length(hosts)){
  temp<-unique(NRRL1counts$Date[NRRL1counts$Condition==hosts[i]])
  print(hosts[i])
  #print(length(temp))
  for (j in 1:length(temp)){
    # make sure n is large enough
    tempCFU<-NRRL1counts$CFU[NRRL1counts$Condition==hosts[i] & NRRL1counts$Date==temp[j]]
    print(length(tempCFU))
  }
}
...
temp<-unique(NRRL1counts$Date[NRRL1counts$Condition==hosts[i]])
  for (j in 1:length(temp)){
    tempCFU<-NRRL1counts$CFU[NRRL1counts$Condition==hosts[i] & NRRL1counts$Date==temp[j]]
    myq<-as.double(quantile(tempCFU, probs=c(0.1, 0.25, 0.5, 0.75, 0.9)))
    All8.big.r2r<-rbind(All8.big.r2r, data.frame(Host=hosts[i],
                                                 Date=temp[j],
                                                 maxCFU=max(tempCFU),
                                                 minCFU=min(tempCFU),
                                                 q90=myq[5],	
                                                 ...
                                                 skewCFU=skewness(tempCFU),
                                                 kurtCFU=kurtosis(tempCFU),
                                                 cvCFU=sqrt(var(tempCFU))/mean(tempCFU)
                                                   ))
}
dim(All8.big.r2r) # 28 x 14
All8.big.r2r[24:28,]

write.table(All8.big.r2r, "WormDataRunToRun.txt", sep="\t")
